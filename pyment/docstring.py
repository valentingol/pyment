...
class DocString(object):
    ...
    def _set_return(self):
        ...
        if type(self.docs["in"]["return"]) is list and self.dst.style["out"] not in [
            ...
            if lst:
                if lst[0][0] is not None:
                    ...
                    if desc[-1] != ".":
                        desc += "."
                    self.docs["out"]["return"] = desc
                self.docs["out"]["rtype"] = lst[0][2]
        else:
            self.docs["out"]["rtype"] = self.docs["in"]["rtype"]
            if (
                isinstance(self.docs["in"]["return"], list)
                and len(self.docs["in"]["return"]) > 0
                and len(self.docs["in"]["return"][0]) > 1
                and self.docs["in"]["return"][0][1] is not None
            ):
                name = self.docs["in"]["return"][0][0]
                desc = self.docs["in"]["return"][0][1]
                desc = desc[0].upper() + desc[1:]
                if desc[-1] != ".":
                    desc += "."
                self.docs["out"]["return"] = desc
                if name is not None and len(name) > 0:
                    rtype = self.docs["in"]["return"][0][2]
                    if rtype is None:
                        rtype = ""
                    self.docs["out"]["rtype"] = name + " : " + rtype

            elif isinstance(self.docs["in"]["return"], str):
                desc = self.docs["in"]["return"]
                desc = desc[0].upper() + desc[1:]
                if desc[-1] != ".":
                    desc += "."
                self.docs["out"]["return"] = desc
        ...
        if self.element["rtype"]:
            if self.docs["out"]["rtype"]:
                rtype_list = self.docs["out"]["rtype"].split(" : ")
                rtype_list[-1] = self.element["rtype"]
                rtype = " : ".join(rtype_list)
        ...
        if self.dst.style["out"] == "numpydoc":
            ...
            for p in self.docs["out"]["params"]:
                ...
                if p[2] is not None and len(p[2]) > 0:
                    raw += " " + p[2]
                    if len(p) >= 4 and p[3] is not None and len(p[3]) > 0:
                        raw += ", optional"
                raw += "\n"
                desc = with_space(p[1]).strip()
                if len(desc) > 1 and ord(desc[0]) in list(range(97, 123)):
                    desc = desc[0].upper() + desc[1:]
                if len(p) > 2:
                    desc = self._handle_default(part=p, desc=desc)
                if len(desc) > 1 and desc[-1] != ".":
                    desc += "."
                raw += self.docs["out"]["spaces"] + spaces + desc
                raw += "\n"
    ...
    def _handle_default(self, part: list, desc: str):
        """Manage default arguments in docstring."""
        templates = [
            "Default to",
            "Defaults to",
            "Defaults:",
        ...
        if "default" in part[1].lower():
            for template in templates:
                desc = desc.replace(template, "By default,")
                desc = desc.replace(", " + template.lower(), ". By default,")
            if "(By default," in desc and desc[-1] == ")":
